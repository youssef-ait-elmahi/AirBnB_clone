...
class TestBaseModel(unittest.TestCase):
    ...
    def test_id_generation(self):
        """Test if the ID is generated correctly for each BaseModel instance."""
        model1 = BaseModel()
        model2 = BaseModel()
        self.assertNotEqual(model1.id, model2.id)

    def test_created_at(self):
        """Test if the `created_at` attribute is set as an instance of datetime."""
        model = BaseModel()
        self.assertIsInstance(model.created_at, datetime)

    def test_updated_at(self):
        """Test if the `updated_at` attribute is updated correctly when calling `save()`."""
        model = BaseModel()
